...
#' @param boundaries how to handle the boundaries, either periodic, reflective or pad. Does nothing if \code{mode="FFT"}
...
my_conv <- function( mat, fil, dec=TRUE, mode="direct", odd=FALSE, boundaries="periodic" ){
    ...
    if( mode == "direct" ){
        if( !( boundaries %in% c( "periodic", "reflective", "pad" ) ) ){ 
            stop( paste0( "unknown boundaries '", boundaries, "', chose 'periodic', 'pad' or 'reflective', friend." ) )
        }
        ...
        if( boundaries == "pad" ){
            bc  <- pad( mat, N=nrow(mat)+length(fil), Ny=ncol(mat) )
        }
        mat <- bc$res
